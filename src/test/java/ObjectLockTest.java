...
public class ObjectLockTest {
    ...
    @Test
    void objectLockTest() throws InterruptedException {
        //given
        ...
        IntStream
                .rangeClosed(1, MAX_TASKS)
                .forEach(x -> generateTask(x, testObject));
    ...
    private void generateTask(int x, TestObject testObject) {
        executor.execute(() -> {
            for (int i = 1; i <= ITERATIONS; i++) {
                System.out.printf("[Thread %d][Iteration %d] is generating id %n", x, i);
                var uniqueId = testObject.getUniqueId();
